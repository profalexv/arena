'use strict';
/**
 * render/proof/proof.js
 *
 * Socket.IO namespace handler para o Proof — avaliação com notas individuais.
 *
...
const logger = {
    info:  (msg) => console.log(`[PROOF][INFO]  ${new Date().toISOString()} - ${msg}`),
    warn:  (msg) => console.warn(`[PROOF][WARN]  ${new Date().toISOString()} - ${msg}`),
    error: (msg) => console.error(`[PROOF][ERROR] ${new Date().toISOString()} - ${msg}`)
};
...
function initializeSocket(nsp) {
    ...
    nsp.on('connection', (socket) => {
        logger.info(`Conexão Proof: ${socket.id}`);
        ...
        socket.on('disconnect', () => {
            ...
            logger.info(`Desconectado do Proof: ${socket.id}`);
        });
    });

    logger.info('Proof Socket.IO Namespace Initialized');
}
