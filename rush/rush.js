...
const logger = {
    info: (msg) => console.log(`[Rush][INFO] ${new Date().toISOString()} - ${msg}`),
    warn: (msg) => console.warn(`[Rush][WARN] ${new Date().toISOString()} - ${msg}`),
    error: (msg) => console.error(`[Rush][ERROR] ${new Date().toISOString()} - ${msg}`)
};

const sessions = {}; // In-memory session storage for Rush
...
function initializeSocket(nsp) {
    ...
    nsp.on('connection', (socket) => {
        logger.info(`User connected to Rush namespace: ${socket.id}`);
        ...
        socket.on('requestJoin', ({ sessionCode, name, password }, callback) => {
            ...
            // In Rush, audience might need a password. Let's assume it's the presenter's password for now.
        ...
        socket.on('disconnect', () => {
            ...
            logger.info(`User disconnected from Rush: ${socket.id}`);
        });

    });

    logger.info("Rush Socket.IO Namespace Initialized");
}
